...
void frontBackSplitLinkedList(LinkedList *ll, LinkedList *resultFrontList, LinkedList *resultBackList)
{
    // 리스트가 비었거나 head가 NULL이면 바로 리턴
    if (ll == NULL || ll->head == NULL)
        return;

    // 앞 리스트에 들어갈 노드 개수 판단(홀수면 앞이 더 많게)
    int mid = (ll->size + 1) / 2;

    // 커서 시작: 원래 리스트의 head부터
    ListNode *cur = ll->head;

    // front 리스트의 시작점은 원래 리스트의 head
    resultFrontList->head = ll->head;

    // mid - 1번 만큼 next를 따라가서 front의 마지막 노드 도착
    for (int i = 0; i < mid - 1; i++) {
        cur = cur->next;
    }

    // 연결 끊기 전에 back의 시작점 백업
    ListNode *backHead = cur->next;
