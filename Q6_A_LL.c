...
int moveMaxToFront(ListNode **ptrHead)
{
    // 리스트가 비었거나 노드가 1개뿐이면 아무 것도 안 함
    if (ptrHead == NULL || *ptrHead == NULL || (*ptrHead)->next == NULL)
        return 0;

    // 초기화
    int maxValue = INT_MIN;         // 가장 큰 값을 저장
    ListNode *maxNode = NULL;       // 가장 큰 값을 가진 노드
    ListNode *maxPrev = NULL;       // maxNode의 바로 이전 노드
    ListNode *cur = *ptrHead;       // 현재 노드를 가리키는 포인터
    ListNode *prev = NULL;          // 현재 노드의 이전 노드를 가리키는 포인터
    ...
    while (cur != NULL) {
        if (cur->item > maxValue) {
            maxValue = cur->item;
            maxNode = cur;
            maxPrev = prev;         // maxNode의 직전 노드 저장
        }
        prev = cur;                 // 다음 루프를 위해 현재 노드를 prev에 저장
        cur = cur->next;           // 다음 노드로 이동
    }

    // maxNode가 이미 맨 앞에 있는 경우
    if (maxPrev == NULL)
        return 0;
